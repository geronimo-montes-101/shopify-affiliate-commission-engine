...
export const action = async ({ request }: ActionFunctionArgs) => {
  ...
  if (!esDominioShopifyValido(shopDomain)) {
    return json({ ok: false, mensaje: "shopDomain invalido." }, { status: 400 });
  }
  ...
  if (!affiliateCode || !orderId || totalAmount === null || subtotalAmount === null) {
    return json(
      {
        ok: false,
        mensaje: "affiliateCode, orderId, totalAmount y subtotalAmount son obligatorios.",
      },
      { status: 400 },
    );
  }
  ...
  if (Number.isNaN(fechaEvento.getTime())) {
    return json({ ok: false, mensaje: "occurredAt invalido." }, { status: 400 });
  }
  ...
  try {
    ...
    if (!afiliado) {
      return json({ ok: false, mensaje: "Afiliado no encontrado." }, { status: 404 });
    }
